...
def getTweets():
    ...
    if data.countTotal > 5:
        result.setData(data.streamingStore.sort_values(by='Fecha de creación',
                                                       ascending=False).iloc[:5, 1:3].to_json(force_ascii=False, orient='records'))
    if data.countTotal > 0 and len(data.streamingStore) <= 5:
        result.setData(data.streamingStore.sort_values(by='Fecha de creación',
                                                       ascending=False).iloc[:, 1:3].to_json(force_ascii=False, orient='records'))
...
def dataForPieChart():
    result = model.chart()
    result.setData([data.countAggresive, data.countNonAggresive,data.countNeutral])
